...
globals$DB_PATH <- system.file("extdata", "pd.primeview.sqlite",
                               package="pd.primeview")
...
pd.primeview <- new("AffyExpressionPDInfo",
                    genomebuild="The Genome Build",
                    getdb=getDb,
                    geometry=as.integer(strsplit("732;732", ";")[[1]]),
                    annotation="pd.primeview")
